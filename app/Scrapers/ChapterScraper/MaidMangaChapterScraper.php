...
class MaidMangaChapterScraper extends BaseChapterScraper
{
    ...
    {
        $linkElement = $xpath->evaluate('.//div[contains(@class, "flexch-infoz")]/a', $element)->item(0);
        if (!$linkElement) {
            Log::error('Chapter extraction failed: No link element found.');
            return null;
        }

        $chapterSpan = $xpath->evaluate('.//a/span/text()[1]', $element)->item(0);
        $chapterTitle = $chapterSpan ? trim($chapterSpan->nodeValue) : '';
    ...
    public function fetchChapters(Manga $manga, string $bucket): array
    {
        ...
        $chapterElements = $xpath->query('//ul[contains(@class, "series-chapterlist")]/li');
